...
/**
     * @param chars - array of char primimtive
     * @param mustBeRules - array of rules that must be true 
     * @param mustNotBeRule - array or rules that must be false
     * @return empty error message if array of chars mathes all 
     * rules otherwise specific error message saying what rules 
     * don't match
     */
public static String matchesRules(char[] chars, CharacterRule[] mustBeRules, CharacterRule[] mustNotBeRule){
    ...
    if (rule.flag && !found) {
        //mustBeRulesCheck[i] = 1;
        result.append(rule.errorMessage).append("\n");
    }
       i++;
       }
      int k = 0;
      while (k<mustNotBeRule.length){
        CharacterRule rule2 = mustNotBeRule[k];
        boolean found_space = false;
        int o = 0;
            while (o<chars.length) {
            if (rule2.predicate.test(chars[o])){
            found_space = true;
            break;
        }
            o++;
    }
    if (!rule2.flag && found_space) {
        result.append(rule2.errorMessage).append("\n");
    }
    k++;
      }
